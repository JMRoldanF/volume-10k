      ...
      * Error message structure
       01  ERROR-MSG.
             ...
             03 FILLER                 PIC X(9)  VALUE ' ZMT00090'.
      ...
      * Called module names
       01  MOD-ZAG01293              PIC X(8) VALUE 'ZAG01293'.
       01  MOD-ZMT02460              PIC X(8) VALUE 'ZMT02460'.
       01  MOD-ZMT00204              PIC X(8) VALUE 'ZMT00204'.
      ...
      *----------------------------------------------------------------*
       MAINLINE SECTION.
               ...
               PERFORM CALL-ZAG01293-001.
               PERFORM CALL-ZMT02460-002.
               PERFORM CALL-ZMT00204-003.
               PERFORM CALL-ZEN00468-004.
      ...
      *----------------------------------------------------------------*
       CALL-ZAG01293-001.
               EXEC CICS LINK PROGRAM('ZAG01293')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZAG01293 FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZMT02460-002.
               EXEC CICS LINK PROGRAM('ZMT02460')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
                         RESP(WS-RESP)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZMT02460 FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZMT00204-003.
               EXEC CICS XCTL PROGRAM('ZMT00204')
                         COMMAREA(DFHCOMMAREA)
                         LENGTH(WS-CALEN)
               END-EXEC.
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZMT00204 FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZEN00468-004.
               EXEC CICS START TRANSID('Z0D0')
                         FROM(WS-KEY-AREA)
                         LENGTH(20)
                         RESP(WS-RESP)
               END-EXEC.
      * TRANSID Z0D0 is defined against ZEN00468
               IF WS-RESP NOT = DFHRESP(NORMAL)
                  MOVE ' LINK ZEN00468 FAILED' TO EM-VARIABLE
                  PERFORM WRITE-ERROR-MESSAGE
               END-IF.
      *----------------------------------------------------------------*
       CALL-ZBI10000-005.
